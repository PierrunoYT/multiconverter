...
const SUPPORTED_FORMATS = {
  image: {
    input: ['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/bmp'],
...
export default function FileConverter() {
  ...
  const handleFiles = useCallback((files: File[]) => {
    const newJobs: ConversionJob[] = files
      .filter(file => SUPPORTED_FORMATS.image.input.includes(file.type))
      .map(file => ({
            ...
            <div key={job.id} className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm border border-gray-200 dark:border-gray-700">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  >
                    {SUPPORTED_FORMATS.image.output.map(format => (
